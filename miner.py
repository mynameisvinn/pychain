...
class Miner():
    ...
    def update_hash(self, new_block, correct_hash):
        """
    ...
    def calc_hash_seal(some_block):
        """
        cryptographic hash function are difficult to reverse, thus the input value of a hash function
        cant be determined from its output value.

        in the case of blockchain, a hash is the "seal" that finalizes the addition of a block to the 
        entire ledger.
        ...
        to make this sealing/hashing function difficult, a seal must satisfy the following condition. 
        a seal, when "mixed" with the block's id number, must result in an output that has three zeros. 
        since this is a NP hard problem (very difficult to find a seal that satisfies that output, 
